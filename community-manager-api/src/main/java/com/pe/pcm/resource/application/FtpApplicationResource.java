 ...
 */
@RestController
@RequestMapping(value = "pcm/application/ftp", produces = APPLICATION_JSON_VALUE)
@Api(tags = {"Application Profile With FTP/FTPS/SFTP"}, description = "FTP/FTPS/SFTP Application Resource")
public class FtpApplicationResource {

    private final FtpApplicationService ftpApplicationService;

    @Autowired
    public FtpApplicationResource(FtpApplicationService ftpApplicationService) {
        this.ftpApplicationService = ftpApplicationService;
    }

    @ApiOperation(value = "Create FTP/FTPS/SFTP Application Profile", authorizations = {@Authorization(value = "apiKey"), @Authorization(value = "basicAuth")})
    @ApiResponses({
    ...
    public ResponseEntity<CommunityManagerResponse> save(@Validated @RequestBody FtpModel ftpModel) {
        ftpApplicationService.save(ftpModel);
        return ResponseEntity.ok(OK.apply(APPLICATION_CREATE));
    }

    @ApiOperation(value = "Update FTP/FTPS/SFTP Application Profile", authorizations = {@Authorization(value = "apiKey"), @Authorization(value = "basicAuth")})
    @ApiResponses({
            @ApiResponse(code = SC_UNAUTHORIZED, message = "unauthorized request"),
            @ApiResponse(code = SC_CONFLICT, message = "resource already exist"),
            @ApiResponse(code = SC_NOT_IMPLEMENTED, message = "protocol not implemented")
    ...
    @ApiOperation(value = "Get FTP/FTPS/SFTP Application Profile", authorizations = {@Authorization(value = "apiKey"), @Authorization(value = "basicAuth")})
    @ApiResponses({
            @ApiResponse(code = SC_UNAUTHORIZED, message = "unauthorized request"),
    ...
    public ResponseEntity<FtpModel> get(@PathVariable String pkId) {
        return ResponseEntity.ok(ftpApplicationService.get(pkId));
    }

    @ApiOperation(value = "Delete FTP/FTPS/SFTP Application Profile", authorizations = {@Authorization(value = "apiKey"), @Authorization(value = "basicAuth")})
    @ApiResponses({
            @ApiResponse(code = SC_UNAUTHORIZED, message = "unauthorized request"),
            @ApiResponse(code = SC_NOT_FOUND, message = "no record found")
    })
