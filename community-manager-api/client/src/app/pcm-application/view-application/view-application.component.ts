...
import {adapterFields, applicationProtocols, protocolAPIS} from 'src/app/model/protocols.constants';
...
export class ViewApplicationComponent implements OnInit {
  ...
  getPkIdDetails(protocol, pkId) {
    ...
    const url = protocolType === 'remote-ftp' ? `/pcm/si/application/${protocolType}/${pkId}` : `${environment.CREATE_APPLICATION}${protocolType}/${pkId}`;
    ...
    this.service.getPkIdDetails(url).subscribe(res => {
      ...
      if (this.applicationForm.value.protocol === 'SFG_CONNECT_DIRECT') {
  ...
  patchPKDetailsValue(res) {
    ...
    if (res.caCertificate && res.caCertificate !== null) {
  ...
  ngOnInit() {
    ...
    this.applicationForm.get('protocol').valueChanges.subscribe(val => {
      ...
      if (val === 'SFGFTP' || val === 'SFGFTPS' || val === 'SFGSFTP') {
    ...
    this.fService.getProtocolList().subscribe(res => {
      ...
      this.applicationFields = this.applicationFields.map(val => {
        if (val.formControlName === 'protocol') {
          ...
          }).filter(type => (type.value !== 'AS2' && type.value !== 'ExistingConnection'));
        }
        return val;
      });
  ...
  submitApplication() {
    ...
    if (!!this.protocol && !!this.pkId) {
      ...
      } else if (partner['protocol'] === 'SFG_CONNECT_DIRECT') {
  ...
  cancel() {
    Swal.fire({
      ...
      confirmButtonText: 'Yes, canecl it!'
    }).then((result) => {
      if (!!result.value) {
        this.router.navigateByUrl('/pcm/application/manage');
      }
