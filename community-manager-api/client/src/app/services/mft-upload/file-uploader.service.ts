...
export class FileUploaderService {
  ...
  public onCompleteItem(queueObj: FileQueueObject, response: any): any {
    this._removeFromQueue(queueObj);
  ...
  private _upload(queueObj: FileQueueObject) {
    ...
    form.append('mailbox', queueObj.file.mailbox)

    // upload file and report progress
    const req = new HttpRequest('POST', environment.MFT_FILE_UPLOAD, form, {headers: headers,
      reportProgress: true
    });
  ...
  mailboxActivity(id, type) {
    return this.http.get(`${environment.MFT_MB_ACTIVITY}${type}?mailbox=${id}`);
  }
}
