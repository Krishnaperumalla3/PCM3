...
export class CreatePartnerComponent implements OnInit {
  ...
  patchPKDetailsValue(res) {
    ...
    if (res.caCertificateNames && res.caCertificateNames !== []) {
    ...
    if (res.knownHostKeyNames && res.knownHostKeyNames !== []) {
    ...
    if (res.protocol !== 'AS2' && res.caCertificate) {
  ...
  submitPartner() {
    ...
    if (!!this.protocol && !!this.pkId) {
      ...
      if(partner['protocol'] === 'GOOGLE_DRIVE') {
        ...
        req = {...partner, ...protocol, ...adapter, ...upload, ...{}};
